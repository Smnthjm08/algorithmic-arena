export function HowItWork (){
   ...
   return (
     <section className="bg-white  bg-gradient-to-t dark:from-[#020817] dark:to-[#0F172A] py-6 md:py-10 ">
       <div className="flex flex-col items-center m-auto text-center gap-4 max-w-[1024px]">
         ...
         <div className="text-sm w-full text-gray-500 md:w-2/3 mx-auto">
  ...
  {steps.map((lang: any, index: number) => (
    <div
      className="border-[1px] rounded-md p-4 text-start gap-2 flex flex-col w-[80%] mx-auto" // Removed conditionals for mr-0 and ml-0
      key={index}
    >
      <div className="flex flex-col gap-1 items-start">
        <div className="flex py-[2px] px-4 rounded-3xl bg-[#3B82F61A] text-blue-500"> 
          Step {index + 1}
        </div>
        <div className="font-bold text-blueGray-50">{lang.heading}</div>
      </div>
